...
Dicha función debe devolver 1 si num es un elemento de datos y 0 en caso contrario. La búsqueda del elemento se 
debe realizar utilizando el algoritmo de búsqueda binaria.
*/

//si el numero es mayor s[dim] ya retorno 0 ya que se que no va a esatr. mismo caso si es menor.
...
int busquedaBinaria(int *v, int dim, int num){
    ...
    return (v[dim/2] > num) ? busquedaBinaria(v,(dim/2)-1, num) : busquedaBinaria(v + (dim/2) + 1, dim - (dim/2) -1, num); 

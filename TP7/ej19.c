...
Si la longitud de s es menor a n debe retornar una copia de s. En caso de que no se 
pueda copiar debe retornar NULL. ¿Qué cuidado debe tener el usuario de esta función?*/
#include <assert.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
char * copiaPrefijo(const char * s, unsigned int n){ 
    int c = strlen(s);
    char * copy = malloc(n+1); //sizeof(char) = 1 por ende no hay probelma.
    if(n >= 0){
    if(c > n){
        for(int i = 0; i < n; i++){
        copy[i] = s[i];
